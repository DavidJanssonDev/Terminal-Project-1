...
class Editor:
    ...
    def get_current_cell(self):
        distance_to_orgin = vector(mouse_pos()) - self.origin
        
        if distance_to_orgin.x > 0:
            col = int(distance_to_orgin.x / TILE_SIZE)
        else:    
            col = int(distance_to_orgin.x / TILE_SIZE) - 1
        
        if distance_to_orgin.y > 0:
            row = int(distance_to_orgin.y / TILE_SIZE)
        else:
            row = int(distance_to_orgin.y / TILE_SIZE) - 1
        
        return col, row
        
    def check_naighbors(self, cell_pos):
        
        #* creat a local cluster 
        cluster_size = 3
        local_cluster = [
    ...
    def imports(self):
        self.water_bottom = load('graphics\\terrain\\water\\water_bottom.png').convert_alpha()
    ...
    def pan_input(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and mouse_buttons()[1]:
            ...
            self.pen_offset = vector(mouse_pos()) - self.origin  
        ...
        #* penning update
        if self.pan_active:
            self.origin = vector(mouse_pos()) - self.pen_offset
    ...
    def canvas_add(self):
        if mouse_buttons()[0] and not self.menu.rect.collidepoint(mouse_pos()) and not self.object_drag_active:
            ...
            if current_cell != self.last_selected_cell:
                ...
                self.check_naighbors(current_cell)
